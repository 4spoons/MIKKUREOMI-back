...
@RestController
@RequestMapping("/api/v1/meal-foods")
...
public class MealFoodController {
    ...
    @PostMapping("/batch")
    public ResponseEntity<List<MealFoodResponseDto>> createMealWithFoods(@RequestBody MealCreateRequestDto requestDto) {
        List<MealFoodResponseDto> responseList = mealFoodService.createMealWithFoods(requestDto);
        return new ResponseEntity<>(responseList, HttpStatus.CREATED); // 201 Created
    }

    /** 2. 특정 Meal에 속한 MealFood 목록 조회 (GET List by Meal ID) */
    @GetMapping("/by-meal/{mealId}")
    public ResponseEntity<List<MealFoodResponseDto>> getMealFoodsByMealId(@PathVariable Long mealId) {
